...
func graphCmd() *cobra.Command {
    ...
    neighbors := &cobra.Command{Use: "neighbors <doc-id>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err := cli.Call(ctx, "graph_neighbors", map[string]interface{}{"doc_id": args[0], "depth": depth}, &res); err != nil { return err }
    ...
    backlinks := &cobra.Command{Use: "backlinks <doc-id>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
        ...
        ctx := context.Background()
        var res []map[string]interface{}
        if err := cli.Call(ctx, "graph_backlinks", map[string]interface{}{"doc_id": args[0]}, &res); err != nil { return err }
        return output.Print(res, cfg.OutputFormat)
    }}

    // Placeholders for future
    path := &cobra.Command{Use: "path <start-id> <end-id>", Args: cobra.ExactArgs(2), RunE: func(cmd *cobra.Command, args []string) error { return output.Print("not implemented", "text") }}
    related := &cobra.Command{Use: "related <doc-id>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error { return output.Print("not implemented", "text") }}
