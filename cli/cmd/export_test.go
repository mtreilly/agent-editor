...
func TestWriteDocsTar(t *testing.T) {
    ...
    docs := []map[string]interface{}{
        {"id": "d1", "slug": "one", "title": "One", "body": "# One"},
    ...
    if err != nil { t.Fatalf("open tar: %v", err) }
    defer f.Close()
    tr := tar.NewReader(f)
    seenDocs := false
    seenMeta := false
    for {
        hdr, err := tr.Next()
        if err == io.EOF { break }
        if err != nil { t.Fatalf("read tar: %v", err) }
        switch hdr.Name {
        case "docs.json":
            seenDocs = true
    ...
    if !seenDocs || !seenMeta {
        t.Fatalf("expected docs.json and meta.json entries")
    }
}
